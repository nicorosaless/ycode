...
/**
 * Migration: grant Supabase roles access to the public schema
 *
...
export async function up(knex: Knex): Promise<void> {
  await knex.schema.raw('GRANT USAGE ON SCHEMA public TO anon, authenticated, service_role');
  await knex.schema.raw('GRANT ALL ON ALL TABLES IN SCHEMA public TO anon, authenticated, service_role');
  await knex.schema.raw('GRANT ALL ON ALL SEQUENCES IN SCHEMA public TO anon, authenticated, service_role');
  await knex.schema.raw('GRANT ALL ON ALL FUNCTIONS IN SCHEMA public TO anon, authenticated, service_role');
...
export async function down(knex: Knex): Promise<void> {
  ...
  await knex.schema.raw('REVOKE ALL ON ALL TABLES IN SCHEMA public FROM anon, authenticated, service_role');
  await knex.schema.raw('REVOKE ALL ON ALL SEQUENCES IN SCHEMA public FROM anon, authenticated, service_role');
  await knex.schema.raw('REVOKE ALL ON ALL FUNCTIONS IN SCHEMA public FROM anon, authenticated, service_role');
  await knex.schema.raw('REVOKE USAGE ON SCHEMA public FROM anon, authenticated, service_role');
}
